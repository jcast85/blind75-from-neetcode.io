...
public class LargestRectangleInHistogramNaive implements LargestRectangleInHistogram {
  ...
  public int largestRectangleArea(int[] heights) {
    ...
    rectangleDataStack.push(new RectangleData(0, 0, 0));
    for(int i=1; i<heights.length; i++) {
      if(heights[i] < heights[i-1]) {
        ...
        rectangleDataStack.push(new RectangleData(newStartIndex, i, i));
      }
      if(heights[i] > heights[i-1]) {
        rectangleDataStack.push(new RectangleData(i, i, i));
      }
    }
  ...
  private static class RectangleData {
    ...
    public RectangleData(int startIndex, int endIndex, int indexMaxHeight) {
